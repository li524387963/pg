...
// 记录开始运行时间
$GLOBALS['_beginTime'] = microtime(TRUE);//microtime返回当前 Unix 时间戳和微秒数
// 记录内存初始使用
define('MEMORY_LIMIT_ON',function_exists('memory_get_usage'));//函数定义了就为真
...
// 系统信息
if(version_compare(PHP_VERSION,'5.4.0','<')) {//对比两个「PHP 规范化」的版本数字字符串
    ini_set('magic_quotes_runtime',0);//为一个配置选项设置值
    define('MAGIC_QUOTES_GPC',get_magic_quotes_gpc()? true : false);
}else{
    define('MAGIC_QUOTES_GPC',false);
}
define('IS_CGI',(0 === strpos(PHP_SAPI,'cgi') || false !== strpos(PHP_SAPI,'fcgi')) ? 1 : 0 );//查找字符串首次出现的位置
define('IS_WIN',strstr(PHP_OS, 'WIN') ? 1 : 0 );//查找字符串的首次出现
...
if(!IS_CLI) {
    ...
    if(!defined('_PHP_FILE_')) {
        if(IS_CGI) {
            //CGI/FASTCGI模式下
            $_temp  = explode('.php',$_SERVER['PHP_SELF']);//使用一个字符串分割另一个字符串
            define('_PHP_FILE_',    rtrim(str_replace($_SERVER['HTTP_HOST'],'',$_temp[0].'.php'),'/'));//去右空格,并子字符串替换
        }else {
            define('_PHP_FILE_',    rtrim($_SERVER['SCRIPT_NAME'],'/'));//去右空格
        }
    }
    if(!defined('__ROOT__')) {
